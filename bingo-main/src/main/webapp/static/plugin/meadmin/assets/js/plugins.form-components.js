		...
		if ($.validator) {
			...
			$.extend($.validator.prototype, {
				resetForm : function() {
					var m = this;
					l.call(this);
					$(this.currentForm).find(".form-group").each(
							function() {
								$(this).removeClass(
										m.settings.errorClass + " "
												+ m.settings.validClass)
							});
					$(this.currentForm).find(".select2-container")
							.removeClass(
									m.settings.errorClass + " "
											+ m.settings.validClass);
					$(this.currentForm).find('label[generated="true"]')
							.html("")
				},
				showLabel : function(n, o) {
					var m = this.errorsFor(n);
					if (m.length) {
						m.removeClass(this.settings.validClass).addClass(
