 ...
 */
package org.bingo.hibernate.query.builder;

import static org.bingo.common.util.StrUtils.concat;
...
public class OqlBuilder<T> extends AbstractQueryBuilder<T> {
	...
	public static <E> OqlBuilder<E> from(final String entityName, final String alias) {
		OqlBuilder<E> query = new OqlBuilder<E>();
		query.entityClass = getEntityClass(entityName);
		query.alias = alias;
	...
	public static <E> OqlBuilder<E> from(final Class<E> entityClass) {
		return from(entityClass, entityClass.getSimpleName());
	}
	
	@SuppressWarnings("unchecked")
	public static <E> Class<E> getEntityClass(String entityName) {
		Class<E> entityClass = null;
			try {
				entityClass = (Class<E>) Class.forName(entityName);
	...
	public static <E> OqlBuilder<E> from(final Class<E> entityClass, final String alias) {
		OqlBuilder<E> query = new OqlBuilder<E>();
		query.entityClass = getEntityClass(entityClass.getName());
		query.alias = alias;
		query.select = "select " + alias;
