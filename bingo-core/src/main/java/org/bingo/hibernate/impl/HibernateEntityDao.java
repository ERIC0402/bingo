 ...
 */
package org.bingo.hibernate.impl;
...
public class HibernateEntityDao extends HibernateDaoSupport implements EntityDao {
	...
	public <T> List<T> get(Class<T> entityClass, String keyName, Collection<?> values) {
		if (entityClass == null || StringUtils.isEmpty(keyName) || values == null || values.isEmpty()) {
			return Collections.emptyList();
		}
		String entityName = entityClass.getName();
	...
	public <T> List<T> get(String entityName, String keyName, Object... values) {
		...
		if (values.length < 500) {
			...
			return (List<T>) search(query.params(parameterMap).build());
		} else {
			QueryBuilder<T> query = OqlBuilder.hql(hql.toString());
	 ...
	 * @see org.bingo.base.entity.query.limit.Pagination
	 */
	public <T> List<T> search(org.bingo.hibernate.model.Query<T> query) {
		if (query instanceof LimitQuery) {
			org.bingo.hibernate.model.LimitQuery<T> limitQuery = (org.bingo.hibernate.model.LimitQuery<T>) query;
			if (null == limitQuery.getLimit()) {
				return QuerySupport.find(limitQuery, getSession());
			} else {
				return new SinglePage<T>(limitQuery.getLimit().getPageNo(), limitQuery.getLimit().getPageSize(), QuerySupport.count(limitQuery, getSession()), QuerySupport.find(query, getSession()));
			}
		} else {
	...
	public <T> T uniqueResult(QueryBuilder<T> builder) {
		List<?> list = (List<?>) search(builder.build());
		if (list.isEmpty()) {
			return null;
		} else if (list.size() == 1) {
			return (T) list.get(0);
	...
	@SuppressWarnings("unchecked")
	public <T> List<T> searchNamedQuery(final String queryName, final Map<String, Object> params, boolean cacheable) {
	...
	public <T> Page<T> paginateQuery(Query query, Map<String, Object> params, PageLimit limit) {
		...
		// 返回结果
		return new SinglePage<T>(limit.getPageNo(), limit.getPageSize(), ((Number) (countQuery.uniqueResult())).intValue(), targetList);
	}
	...
	private void saveEntity(Object entity, String entityName) {
		...
		if (null != entityName) {
			...
			if (entity instanceof HibernateProxy) {
				...
				getHibernateTemplate().save(entity.getClass().getName(), entity);
			}
		}
	}
	...
	private void persistEntity(Object entity, String entityName) {
		...
		if (null != entityName) {
			...
			if (entity instanceof HibernateProxy) {
				...
				getHibernateTemplate().saveOrUpdate(entity.getClass().getName(), entity);
			}
		}
	}
	...
	public static final class QuerySupport {
		...
		private static Query buildHibernateQuery(org.bingo.hibernate.model.Query<?> bquery, final Session hibernateSession) {
			Query hibernateQuery = null;
			if (bquery.getLang().equals(org.bingo.hibernate.model.query.Lang.HQL)) {
		 ...
		 * @return
		 */
		public static int count(final org.bingo.hibernate.model.LimitQuery<?> limitQuery, final Session hibernateSession) {
			final org.bingo.hibernate.model.Query<?> cntQuery = limitQuery.getCountQuery();
			if (null == cntQuery) {
		 ...
		 */
		@SuppressWarnings("unchecked")
		public static <T> List<T> find(final org.bingo.hibernate.model.Query<T> query, final Session hibernateSession) {
			if (query instanceof LimitQuery<?>) {
